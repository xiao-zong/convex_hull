...
\title{
\huge
 Subgraph counts in random-connection model with endpoints
} 
...
\begin{abstract} 
This is a short note for \cite{LiuPrivault}. The notation and results considered here are supposed to be consistent and self-contained in \cite{LiuPrivault}.
\end{abstract}
...
the set of connected graphs whose vertex sets are non-flat partitions of
 $[n]\times [r]$, $n, r\geq 1$. 
 Next, we recall some related concepts from random graph theory, see \cite{luczak,JLR}.  
\begin{definition} \cite[pages~64-65]{JLR}
  \begin{enumerate}[1)]
    \item
...
\begin{remark}\label{graph-example}
 As pointed out in \cite{luczak}, strongly balanced graphs are strictly balanced. Also, trees are $K_2$-balanced but not strictly $K_2$-balanced. Cycles and complete graphs are strictly $K_2$-balanced. Moreover, every $K_2$-balanced graph except unions of disjoint edges is strongly balanced.
\end{remark}  
\begin{lemma}
...
% The terminology is extending from random graph theory, \cite{luczak,JLR}, where it is defined only for variance term, i.e. when $n=2$.
...
\begin{Proof}
 ...
 Therefore, we have 
    ...
    \end{eqnarray*}
    and we conclude by Theorem~6.14 in \cite{JLR}. 
\end{Proof}
...
\end{equation}
for any non-negative integer-valued random variable $X$, see \cite[Page~54]{JLR}, we consider the thresholds for subgraph containment. 
\begin{thm}
...
\end{equation}
\end{assumption}
It has been shown in \cite{LiuPrivault} that the Rayleigh connection function $H(x,y):=e^{-\beta\|x-y\|^2}$, for some $\beta>0$, satisfies Assumption~\ref{measurability1}. 
...
A $k$-hop $P_k$ is a path with $k$ edges. Due to its simple structure, $k$-hop counts has wide applications in wireless communication, c.f. \cite{prkhp}. We can see that the $k$-hop count $N_{y}^{P_k}$, with one endpoint, and $N_{y_1,y_2}^{P_k}$, with two endpoints, both satisfy that the condition in Proposition~\ref{lm:slope2}, i.e. the upper boundary is a line segment. 
